...
public class ExcellWrite {
    ...
    XSSFSheet sheet;
    @Test
    public void excell_write_test() throws IOException {
        ...
        for (int rowNum=0;rowNum<sheet.getLastRowNum();rowNum++){
            if (sheet.getRow(rowNum).getCell(0).toString().equals("Mary")){
               sheet.getRow(rowNum).getCell(1).setCellValue("Jonesss");
            }
        }
        ...
        //save changes              : File output stream--> writing

        FileOutputStream outputStream=new FileOutputStream(filePath);

        //save the changes on workbook
